...
import com.originsdigital.compositeadapter.messages.ui.cell.databinding.SampleDataBindingMessageCell
import com.originsdigital.compositeadapter.messages.ui.cell.view.SampleViewMessageCell
import com.originsdigital.compositeadapter.messages.ui.cell.viewbinding.SampleViewBindingMessageCell
...
class SampleMapper(
    ...
    init {
        ...
        firstItemDecoration =
            SpaceItemDecoration(top = 8 * space, bottom = space, start = 8 * space, end = 8 * space)
        ...
        middleBigItemDecoration =
            SpaceItemDecoration(top = space, bottom = space, start = space, end = space)
        lastItemDecoration =
            SpaceItemDecoration(top = space, bottom = 8 * space, start = 8 * space, end = 8 * space)
    }
    ...
    fun getLoaderCell(fullscreen: Boolean): Cell<*> {
        return if (fullscreen) {
            CommonFullLoaderCell()
        } else {
            CommonLoaderCell(
                data = CommonLoaderUI("", heightId = null),
                decoration = null
            )
        }
    }
    ...
    fun getErrorCell(
        ...
        val mapper = ErrorUIMapper()
        return if (fullscreen) {
            CommonFullErrorCell(
                data = mapper.mapError(id = "messages", text = error),
                decoration = null,
